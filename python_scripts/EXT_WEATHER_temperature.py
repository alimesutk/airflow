...
df_weather_temperature = pd.read_csv("/opt/airflow/datasets/weather/temperature.csv")

df_db_weather_temperature = df_weather_temperature.iloc[:, 0:37]
df_db_weather_temperature.columns = ['datetime', 'Vancouver', 'Portland', 'San_Francisco', 'Seattle', 'Los_Angeles',
...
df_weather_temperature_unpivot = df_db_weather_temperature.melt(id_vars='datetime', var_name='Cities', value_name='temperature')

print(df_weather_temperature_unpivot)
...
class weather_temperature(Base):
    """Data model weather_temperature."""
    ...
    temperature_sk = Column('temperature_sk', Integer, primary_key=True, autoincrement=True)
    datetime = Column('datetime', TIMESTAMP, nullable=True)
    cities = Column('cities', String(200), nullable=True)
    temperature = Column('temperature', Numeric, nullable=True)


# CREATE TABLE
if not engine.dialect.has_table(engine, weather_temperature.__tablename__):
    Base.metadata.create_all(engine)

df_weather_temperature_unpivot.to_sql(weather_temperature.__tablename__, engine, index=False,
